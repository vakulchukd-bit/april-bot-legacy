...
def scene_contract_get(contract, key, default=None):
    if contract is None:
        return default
    if isinstance(contract, dict):
...
def normalize_executor_response(
    ...
    normalized = {
        ...
        "blocks_present":
            bool(
                scene_contract_get(result.get("scene_contract"), "render_blocks", [])
        ...
        "scene":
            safe_json(scene_contract_get(result.get("scene_contract"), "scene", result.get("scene", {}))),
