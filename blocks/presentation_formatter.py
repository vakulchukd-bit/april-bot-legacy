...
from blocks.interpretation_layer import interpret_request, build_processor_execution_context, QUANTUM_EVIDENCE_FUSION, QUANTUM_DIALOGUE_ENGINE
...
def _usable_visual_scene(state: dict) -> dict:
    ...
    order = (('current_visual_scene', 0), ('active_visual_scene', 1), ('active_scene_contract', 2), ('last_successful_visual_scene', 3))
...
def _best_visual_context(state: dict) -> dict:
    ...
    for key in ('active_scene_contract', 'current_visual_scene', 'active_visual_scene'):
        ...
        if not isinstance(blocks, list):
            continue
        if any((isinstance(block, dict) and _s(block.get('type') or block.get('artifact_type') or block.get('representation')).lower() not in {'', 'text', 'markdown'} for block in blocks)):
            result = deepcopy(candidate)
...
def _preserve_semantic_visual_representation(text: str, semantic: dict, control_relation: str, state: dict, requested_outputs: list[str], preferred: str) -> tuple[list[str], str, dict[str, Any]]:
    ...
    should_preserve = relation in {'ARTIFACT_REFERENCE', 'MEMORY_QUERY'} and (not artifact_reference_answer) or (operation in allowed_operations and (not artifact_reference_answer))
    ...
    def clean(values):
        ...
        for raw in _as_list(values):
            ...
            if name and name not in internal and (name not in result):
                result.append(name)
        return result
    canonical = _s(semantic.get('production_representation') or semantic.get('resolved_representation') or semantic.get('requested_representation') or semantic.get('preferred_representation') or decision.get('preferred_representation')).lower()
    canonical = aliases.get(canonical, canonical)
    declared = clean(semantic.get('requested_outputs') or semantic.get('required_outputs') or semantic.get('requested_representations') or semantic.get('required_representations'))
...
def _build_processor_control_plane(*, text: str, semantic: dict, cognition: dict, decision: dict, state: dict, dynamic_memory: dict | None=None, memory_understanding: dict | None=None) -> dict:
    ...
    canonical_dialogue = _as_dict(interpretation_packet.get('dialogue_contract') or semantic.get('dialogue_context_field'))
    ...
    resolved_memory_reference = bool(memory_resolved and memory_scene_id and current_scene_id and (memory_scene_id == current_scene_id) and memory_reference.get('target'))
    ...
    if not (reference_to_previous or continuation):
        ...
        if _s(current_scene.get('scene_id')) == _s(memory_scene.get('scene_id')):
            resolved_scene = {**resolved_scene, 'scene_id': _s(memory_scene.get('scene_id')), 'relation': 'current_scene', 'source': 'current_visual_scene'}
    relation = _s(resolved_scene.get('relation'))
    if not relation:
        relation = 'current_scene' if continuation or reference_to_previous or mode == 'MEMORY_QUERY' else 'new_topic' if mode == 'NEW_TOPIC' else 'independent'
    outputs, preferred = _canonical_requested_outputs(text, semantic, decision, mode)
    representation_state = {'outputs': list(outputs), 'preferred': preferred, 'selection_method': 'interpretation_canonical_production', 'scoring': False, 'triggers': False}
    ...
    constraints = _representation_constraints(semantic, cognition, decision)
    topic = _s((memory_reference.get('target') if reference_to_previous else '') or (memory_dialogue.get('active_topic') if reference_to_previous else '') or canonical_dialogue.get('active_topic') or _field((semantic, decision, state), ('active_topic', 'topic', 'current_topic')))
    goal = _s((memory_resolved_request if reference_to_previous else '') or canonical_dialogue.get('active_goal') or _field((decision, cognition, semantic), ('active_goal', 'resolved_request', 'goal'))) or text
    capabilities: list[str] = []
    ...
    control = {'version': 'QUANTUM_CONTROL_PLANE_V1', 'authority': {'dialogue': 'interpretation', 'representation': 'semantic_decision', 'capabilities': 'semantic_cognition', 'memory': 'state_manager', 'production': 'executor_specialized_engines', 'presentation': 'executor_presentation_matrix', 'rendering': 'april_web'}, 'mode': mode, 'relation': relation, 'continuation': continuation, 'reference_to_previous': reference_to_previous, 'context_dependency': context_dependency, 'resolved_scene': resolved_scene, 'active_topic': topic, 'visual_continuity': continuity_representation, 'active_goal': goal, 'dialogue_evidence': evidence, 'requested_outputs': outputs, 'preferred_representation': preferred, 'representation_state': representation_state, 'representation_constraints': constraints, 'geometry_contract': _canonical_geometry_contract(semantic, text) if preferred == 'diagram' or 'diagram' in outputs else {}, 'capabilities': capabilities[:12], 'dynamic_memory': dynamic_memory if isinstance(dynamic_memory, dict) else {}, 'memory_understanding': _quantum_snapshot(memory_understanding or {}), 'resolved_request': memory_resolved_request if reference_to_previous else _s(text), 'resolved_reference': _quantum_snapshot(memory_reference if reference_to_previous else {}), 'single_route': True, 'provider_calls': 1, 'triggers': False, 'score_routing': False}
...
def _make_request(text: str, semantic: dict, cognition: dict, decision: dict, state: dict, visual: dict, control: dict | None=None) -> MachineRequest:
    ...
    dialogue_contract = {'dialog_act': _s(dialogue_contract_source.get('dialog_act') or _field((semantic, decision, cognition), ('dialog_act', 'dialogue_act'))) or 'statement', 'continuation': bool(control.get('continuation')), 'reference_to_previous': bool(control.get('reference_to_previous')), 'context_dependency': _s(dialogue_contract_source.get('context_dependency')) or ('continuation' if mode == 'CONTINUATION' else 'reference' if mode == 'ARTIFACT_REFERENCE' else 'independent' if mode == 'INDEPENDENT' else 'topic'), 'reply_to': _s(_field((dialogue_contract_source, semantic, decision), ('reply_to', 'previous_turn_id'))), 'previous_user_turn': _s(dialogue_contract_source.get('previous_user_turn') or evidence.get('previous_user')), 'previous_april_turn': _s(dialogue_contract_source.get('previous_april_turn') or evidence.get('previous_april')), 'active_goal': _s(dialogue_contract_source.get('active_goal')) or _s(control.get('active_goal')), 'active_topic': _s(dialogue_contract_source.get('active_topic')) or _s(control.get('active_topic')), 'resolved_scene': _as_dict(dialogue_contract_source.get('resolved_scene')), 'current_request': _s(text)}
    ...
    request_metadata = {'processor_version': PROCESSOR_VERSION, 'assistant_identity': deepcopy(APRIL_IDENTITY), 'assistant_identity_name': APRIL_IDENTITY.get('name', 'April'), 'identity_request': bool(semantic.get('identity_request')), 'single_route': True, 'provider_calls_per_request': 1, 'context_mode': mode, 'dialogue_coherence': round(coherence, 4), 'identity_scope': deepcopy(scope), 'control_plane_version': control.get('version')}
    if isinstance(state, dict):
    ...
    request = MachineRequest(goal=_s(control.get('active_goal')) or text, intent={'type': _s(semantic.get('intent')) or ('self_identification' if semantic.get('identity_request') else 'dialogue'), 'normalized_text': _s(text), 'dialogue_state': mode, 'coherence': round(coherence, 4), 'dialog_act': dialogue_contract['dialog_act']}, conversation={'current_request': _s(text), 'dialogue_contract': dialogue_contract, 'dialogue_vector': deepcopy(semantic.get('dialogue_vector') or {}), 'dialogue_delta': deepcopy(semantic.get('dialogue_delta') or {}), 'render_continuity': deepcopy(semantic.get('render_continuity') or {}), 'visual_schema': _s(semantic.get('visual_schema')), 'visual_schema_confidence': float(semantic.get('visual_schema_confidence') or 0.0), 'context_mode': mode, 'context_dependency': bool(control.get('context_dependency')), 'resolved_request': _s(dialogue_contract.get('resolved_request') or dialogue_contract_source.get('resolved_request') or text), 'previous_user_turn': _s(dialogue_contract_source.get('previous_user_turn') or evidence.get('previous_user')), 'previous_april_turn': _s(dialogue_contract_source.get('previous_april_turn') or evidence.get('previous_april')), 'resolved_scene': _as_dict(dialogue_contract.get('resolved_scene') or dialogue_contract_source.get('resolved_scene') or control.get('resolved_scene')), **({'active_topic': _clip(_s(control.get('active_topic')), 300), 'active_goal': _clip(_s(control.get('active_goal')), 500)} if mode != 'INDEPENDENT' else {}), **({'recent_dialogue': context.get('recent_dialogue', [])} if mode in {'CONTINUATION', 'SAME_TOPIC', 'ARTIFACT_REFERENCE', 'MEMORY_QUERY'} or bool(control.get('context_dependency')) else {})}, memory={'active_topic': _clip(_s(control.get('active_topic')), 300), 'active_goal': _clip(_s(control.get('active_goal')), 500), 'active_scene_id': _s(_as_dict(dialogue_evidence.get('scene_continuity')).get('scene_id') or _as_dict(_best_visual_context(state)).get('scene_id')), 'retrieval_mode': 'memory_query' if mode == 'MEMORY_QUERY' else 'semantic', 'dynamic_memory': dynamic_memory_evidence if mode in {'CONTINUATION', 'SAME_TOPIC', 'ARTIFACT_REFERENCE', 'MEMORY_QUERY'} or bool(control.get('reference_to_previous')) else {'available': bool(dynamic_memory_evidence.get('matches'))}} if mode != 'INDEPENDENT' else {'active_scene_id': _s(_as_dict(_best_visual_context(state)).get('scene_id'))}, visual_context=visual if isinstance(visual, dict) and (mode in {'CONTINUATION', 'SAME_TOPIC', 'ARTIFACT_REFERENCE', 'MEMORY_QUERY'} or bool(_best_visual_context(state))) else {}, available_tools=list(control.get('capabilities') or []), requested_outputs=requested_outputs, required_competencies=list(control.get('capabilities') or []), required_artifacts=requested_outputs, routing={'single_route': True, 'processor': PROCESSOR_VERSION, 'measured_state': mode, 'identity_scope': deepcopy(scope)}, constraints={'one_provider_call': True, 'one_visible_answer': True, 'canonical_scene': True, 'dialogue_coherence': round(coherence, 4), 'quantum_state': {'dialogue': dialogue_state, 'representation': control.get('representation_state', {}), 'measured_output': measured_output}, 'provider_input_token_budget': 900, 'provider_context_strategy': 'provider_router_semantic_field_selection', 'current_request_must_remain_intact': True, 'identity_scope': deepcopy(scope), 'presentation_plan': presentation_plan, 'quantum_context_diagnostic': _quantum_snapshot(semantic.get('quantum_context_diagnostic') or {}), 'representation_plan': {'requested_outputs': requested_outputs, 'preferred_representation': measured_output, 'geometry_contract': control.get('geometry_contract', {}) if measured_output == 'diagram' or 'diagram' in requested_outputs else {}, 'visual_schema': _s(semantic.get('visual_schema')), 'visual_schema_confidence': float(semantic.get('visual_schema_confidence') or 0.0), 'dialogue_relation': _s(semantic.get('dialogue_relation')) or 'NEW_TOPIC', 'dialogue_subtype': _s(semantic.get('dialogue_subtype')) or 'NEW_TOPIC', 'avoid_repeat': True, 'constraints': representation_constraints, 'audit': representation_audit, 'current_request_authoritative': True}, 'metadata': request_metadata})
    ...
    request.quantum_state = {'dialogue': dialogue_state, 'representation': control.get('representation_state', {}), 'measured_output': measured_output, 'geometry_contract': control.get('geometry_contract', {}) if measured_output == 'diagram' or 'diagram' in requested_outputs else {}, 'context_dependency': bool(control.get('context_dependency')), 'reference_to_previous': bool(control.get('reference_to_previous')), 'continuation': bool(control.get('continuation')), 'scene_continuity': _quantum_snapshot(control.get('dialogue_evidence', {}).get('scene_continuity', {})), 'evidence_channels': len(evidence), 'coherence': round(coherence, 4), 'response_budget': response_budget, 'response_budget_min': OUTPUT_MIN_TOKENS, 'response_budget_max': OUTPUT_MAX_TOKENS, 'response_budget_mode': 'continuous_64_signal_scale', 'quantum_cores': QUANTUM_CORE_COUNT, 'quantum_lanes': QUANTUM_LANE_COUNT, 'quantum_signal_count': QUANTUM_CORE_COUNT * QUANTUM_LANE_COUNT, 'quantum_budget_field': quantum_budget_field, 'response_budget_logical': True, 'response_budget_compression_ceiling': OUTPUT_MAX_TOKENS, 'control_plane': _quantum_snapshot(control)}
    ...
    request.constraints['metadata'].update({'processor_version': PROCESSOR_VERSION, 'visual_context_evidence': bool(visual), 'single_route': True, 'provider_calls_per_request': 1, 'context_mode': mode, 'dialogue_coherence': round(coherence, 4), 'response_budget': response_budget, 'response_budget_min': OUTPUT_MIN_TOKENS, 'response_budget_max': OUTPUT_MAX_TOKENS, 'response_budget_mode': 'continuous_64_signal_scale', 'quantum_cores': QUANTUM_CORE_COUNT, 'quantum_lanes': QUANTUM_LANE_COUNT, 'quantum_signal_count': QUANTUM_CORE_COUNT * QUANTUM_LANE_COUNT, 'quantum_budget_field': quantum_budget_field, 'requested_outputs': requested_outputs, 'identity_scope': deepcopy(scope), 'control_plane': _quantum_snapshot(control), 'presentation_plan': _quantum_snapshot(request.constraints.get('presentation_plan', {})), 'representation_plan': _quantum_snapshot(request.constraints.get('representation_plan', {}))})
    return request
...
def _ensure_visible_text_block(blocks: Any, answer: str, *, source: str='quantum_processor') -> list[dict]:
    ...
    if not answer_text:
        return canonical
    text_block = {'type': 'text', 'artifact_type': 'text', 'content': answer_text, 'text': answer_text, 'renderer': 'TextBlock', 'viewer': 'TextBlock', 'scene_contract': True, 'human_visible': True, 'presentation_role': 'answer', 'source': source}
...
def _apply_new_dataset_dialogue_boundary(interpretation: dict[str, Any], current_text: str, state: dict[str, Any]) -> dict[str, Any]:
    ...
    vector.update({'relation': 'INDEPENDENT', 'subtype': 'NEW_STRUCTURED_DATASET', 'continuation_score': 0.0, 'independent_score': 1.0, 'relation_strength': 1.0, 'continuation': False, 'reference_to_previous': False, 'delta_mode': 'start', 'avoid_repeat': False, 'reuse_existing_scene': False, 'previous_scene_id': '', 'previous_render_types': [], 'previous_block_ids': [], 'reference_resolution': {'resolved': False, 'target': '', 'confidence': 1.0, 'source': 'QUANTUM_STRUCTURED_PAYLOAD_BOUNDARY'}, 'resolved_reference': '', 'resolved_request': current_text, 'source': 'quantum_structured_payload_boundary_v1', 'decision_owner': 'QUANTUM_PROCESSOR'})
    interpretation['dialogue_vector'] = vector
    interpretation['render_continuity'] = {'relation': 'NEW_TOPIC', 'reuse_existing_scene': False, 'previous_scene_id': '', 'previous_render_types': [], 'avoid_repeat': False}
    interpretation['dialogue_contract'] = {**_as_dict(interpretation.get('dialogue_contract')), 'context_mode': 'INDEPENDENT', 'dialogue_state': 'INDEPENDENT', 'continuation': False, 'reference_to_previous': False, 'context_dependency': 'independent', 'current_request': current_text, 'resolved_request': current_text, 'active_topic': '', 'active_goal': current_text, 'resolved_scene': {}}
    interpretation['quantum_scene_continuity'] = {**_as_dict(interpretation.get('quantum_scene_continuity')), 'canonical': True, 'mode': 'INDEPENDENT', 'continuation': False, 'reference_to_previous': False, 'dialogue_label': 'independent', 'scene_id': '', 'previous_user': '', 'previous_april': '', 'source': 'QUANTUM_STRUCTURED_PAYLOAD_BOUNDARY', 'structured_payload_relation': relation}
...
def _extract_label_value_pairs(text: str) -> list[tuple[str, float, str]]:
    ...
    for match in re.finditer(year_pattern, source):
        ...
        unit = _s(match.group(3))
        pairs.append((label, value, unit))
    generic_pattern = '(?m)^\\s*(?:[-*•]\\s*)?([A-Za-zА-Яа-яЁё][A-Za-zА-Яа-яЁё0-9 _]{1,30}?)\\s*(?:[—–:-])\\s*\\$?\\s*(-?\\d+(?:[.,]\\d+)?)\\s*(%|°\\s*[CF]|[A-Za-zА-Яа-яЁё$€£]+)?\\s*$'
...
def _ensure_quantum_structured_outputs(response: MachineResponse, request: MachineRequest | None) -> MachineResponse:
    ...
    graph_block = next((b for b in blocks if isinstance(b, dict) and _s(b.get('type') or b.get('artifact_type') or b.get('representation')).lower() == 'graph'), None)
    ...
    def _usable_graph_payload(payload: Any) -> bool:
        ...
        if isinstance(series, list):
            ...
            for item in series:
                ...
                if isinstance(points, list) and any((isinstance(p, dict) and p.get('x') not in (None, '') and (p.get('y') not in (None, '')) for p in points)):
                    return True
    ...
    if graph_block is not None and (not existing_valid):
        blocks = [block for block in blocks if not (isinstance(block, dict) and _s(block.get('type') or block.get('artifact_type') or block.get('representation')).lower() == 'graph')]
    current_text = _s(request.conversation.get('current_request') if isinstance(request.conversation, dict) else '')
    ...
    if not payload:
        return response
    graph = {'type': 'graph', 'artifact_type': 'graph', 'renderer': 'line_chart', 'viewer': 'line_chart', 'content': '', 'text': '', 'payload': payload, 'artifact': {'type': 'graph', 'renderer': 'line_chart', 'viewer': 'chart', 'scene_contract': True, 'payload': deepcopy(payload)}, 'scene_contract': True, 'provider_payload': True, 'canonical_provider_payload': True, 'source': 'QUANTUM_STRUCTURED_OUTPUT_ENGINE'}
    blocks.append(graph)
    response.render_blocks = blocks
    meta = dict(getattr(response, 'metadata', {}) or {})
    meta['quantum_structured_output_engine'] = {'version': 'V1', 'materialized': ['graph'], 'source': 'explicit_structured_data_and_visual_context', 'natural_language_triggering': False, 'payload_preserved': True}
...
def _canonicalize(user_id: str, response: MachineResponse, state: dict, semantic: dict, cognition: dict, decision: dict, request: MachineRequest, internal_context: bool=False) -> dict:
    ...
    try:
        ...
        contract.metadata['presentation_stream'] = {'version': 'quantum_presentation_stream_v1', 'nodes': stream, 'single_visible_stream': True, 'answer_is_fallback': True}
    except Exception:
        pass
    try:
        contract.metadata = dict(contract.metadata or {})
...
async def execute(user_id, chat_id=None, text='', run_with_activity=None, **kwargs):
    ...
    semantic = semantic_analyze(text=text, state=state, history=history, active_flow=active_flow, dialog_state=dialog_state, interpreted=interpretation) or {}
    ...
    request_meta = _request_metadata(request)
    request_meta.update({'dynamic_memory_available': bool(dynamic_memory.get('matches')), 'dynamic_memory_match_count': len(dynamic_memory.get('matches') or []), 'quantum_evidence_channels': 15, 'quantum_evidence_field_version': PROCESSOR_VERSION, 'provider_calls_per_request': 1, 'single_route': True, 'requested_outputs': list(request.requested_outputs), 'dialogue_vector': _quantum_snapshot(interpretation.get('dialogue_vector', {})), 'dialogue_delta': _quantum_snapshot(interpretation.get('dialogue_delta', {})), 'render_continuity': _quantum_snapshot(interpretation.get('render_continuity', {})), 'representation_plan': _quantum_snapshot(request.constraints.get('representation_plan', {})), 'representation_audit': _quantum_snapshot(request.constraints.get('representation_plan', {}).get('audit', {})), 'processor_context': processor_context, 'memory_understanding': _quantum_snapshot(memory_understanding), 'quantum_context_diagnostic': _quantum_snapshot(semantic.get('quantum_context_diagnostic') or {})})
    ...
    request.constraints.setdefault('metadata', {})['quantum_release_audit'] = {'evidence_channels': 15, 'decision_owner': 'QUANTUM_PROCESSOR', 'single_route': True, 'provider_calls': 1, 'response_budget': getattr(request, 'response_output_tokens', 0), 'response_budget_range': [OUTPUT_MIN_TOKENS, OUTPUT_MAX_TOKENS], 'response_budget_canonical': True, 'quantum_cores': 8, 'quantum_lanes_per_core': 8, 'quantum_signal_count': 64, 'response_budget_mode': 'continuous_64_signal_scale', 'input_budget': 900, 'input_budget_mode': 'logical_compaction', 'quantum_semantic_engines': ['spacy_linguistic', 'sentence_transformers_embedding', 'transformers_nli', 'context_vector_fusion'], 'word_trigger_routing': False, 'fallback_semantics': False, 'quantum_budget_field': quantum_budget_field, 'experience': True, 'experience_manager': True, 'goal_engine': True, 'visual_reference_system': True, 'control_plane_version': control_plane.get('version'), 'control_plane_single_route': bool(control_plane.get('single_route'))}
